...
class Model:
    def __init__(self, input_size_1, input_size_2, save_path, epoch):
        self.model = None
    ...
    def build_model(self, hp):
        ...
        hp_neurons = hp.Choice('dense_neurons_1', values=[32,64,128,192,256])
        #hp_neurons = hp.Int('dense_neurons_1', min_value=64, max_value=256, step=64)
        hp_activation = hp.Choice('activation', values=['relu', 'tanh'])
        model.add(Dense(units=hp_neurons, activation=hp_activation))
...
model_y_mntm = Model(24, 1, 'model_y_mntm/', 20)

model_y_mntm.hyperparameter_search(train_x, train_y, val_x, val_y)

model_y_mntm.print_summary()
